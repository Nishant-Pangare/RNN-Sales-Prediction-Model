...
#             predictions[next_day_date.date()] = next_day_sales[0][0]
#         return predictions
#     except Exception as e:
#         print(f"Error: {e}")
#         return None
...
#                                     # Predictions
#                                     predictions_week = predict_next_week(trained_model, scaler, scaled_data, df, max_date)
#                                     predictions_month = predict_next_month(trained_model, scaler, scaled_data, df, max_date)

#                                     # Convert keys to strings
#                                     predictions_week = {str(key): value for key, value in predictions_week.items()}
#                                     predictions_month = {str(key): value for key, value in predictions_month.items()}

#                                     # Return JSON response
#                                     return jsonify({"Predictions for the next week": predictions_week, "Predictions for the next month": predictions_month})
#                                 else:
#                                     return jsonify({"error": "Error occurred during model training."})
#                             else:
#                                 return jsonify({"error": "Error occurred during model building."})
#                         else:
#                             return jsonify({"error": "Error occurred during data preparation for training."})
#                     else:
#                         return jsonify({"error": "Error occurred during data scaling."})
#                 else:
#                     return jsonify({"error": "Error occurred during data preprocessing."})
#             else:
#                 return jsonify({"error": "Error occurred during loading data into DataFrame."})
#         else:
#             return jsonify({"error": "Error occurred during fetching data from MySQL database."})
...
def predict_next_week(model, scaler, scaled_data, df, max_date):
    try:
        ...
        for i in range(7):
            ...
            next_day_date = max_date + pd.DateOffset(days=i+1)
            print(f"Predicted Total Sales for {next_day_date.date()}: {next_day_sales[0][0]:.2f}")
            predictions[next_day_date.date()] = float(next_day_sales[0][0])  # Convert to float
        return predictions
    except Exception as e:
        print(f"Error: {e}")
        return None
